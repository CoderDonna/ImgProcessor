...
const packageNames = await fs.readdir(new URL('../packages/@ImgProcessor', import.meta.url))
...
const { packages } = await prompts({
  ...
  hint: '@ImgProcessor/core is automatically included',
...
const app = dedent`
    ...
    ${packages.map((pgk) => `import ${camelcase(pgk)} from '@ImgProcessor/${pgk}'`).join('\n')}
...
const prettyPath = (url) => url.toString().split('ImgProcessor', 2)[1]
