describe('Dashboard with Transloadit', () => {
  ...
  it('should close assembly polling when cancelled', () => {
    ...
    cy.window().then(({ ImgProcessor }) => {
      ...
      cy.wait(['@createAssemblies']).then(() => {
        ...
        // @ts-ignore fix me
        expect(
          Object.values(ImgProcessor.getPlugin('Transloadit').activeAssemblies).every(
            (a: any) => a.pollInterval,
          ),
        ).to.equal(true)

        ImgProcessor.cancelAll()

        cy.wait(['@delete']).then(() => {
          // eslint-disable-next-line
          // @ts-ignore fix me
          expect(
            Object.values(ImgProcessor.getPlugin('Transloadit').activeAssemblies).some(
              (a: any) => a.pollInterval,
            ),
          ).to.equal(false)
        })
  ...
  it.skip('should emit one assembly-cancelled event when cancelled', () => {
    ...
    cy.window().then(({ ImgProcessor }) => {
      // eslint-disable-next-line
      // @ts-ignore fix me
      ImgProcessor.on('transloadit:assembly-cancelled', spy)
  ...
  it.skip('should close assembly polling when all files are removed', () => {
    ...
    cy.window().then(({ ImgProcessor }) => {
      ...
      const { files } = ImgProcessor.getState()
      // eslint-disable-next-line
      // @ts-ignore fix me
      ImgProcessor.removeFiles(Object.keys(files))
  ...
  it('should not create assembly when all individual files have been cancelled', () => {
    ...
    cy.window().then(({ ImgProcessor }) => {
      ...
      // @ts-ignore fix me
      expect(
        Object.values(ImgProcessor.getPlugin('Transloadit').activeAssemblies).length,
      ).to.equal(0)
  ...
  it.skip('should create assembly if there is still one file to upload', () => {
    ...
    cy.window().then(({ ImgProcessor }) => {
      ...
      ImgProcessor.removeFile(fileID)
  ...
  it('should not emit error if upload is cancelled right away', () => {
    ...
    cy.window().then(({ ImgProcessor }) => {
      const { files } = ImgProcessor.getState()
      ImgProcessor.on('upload-error', handler)
  ...
  it('should not re-use erroneous tus keys', () => {
    function createAssemblyStatus({
      ...
      return {
        ...
        ImgProcessorserver_url: 'https://api2-test.transloadit.com/companion/',
        ...
        client_referer: null,
        transloadit_client:
          'ImgProcessor-core:3.2.0,ImgProcessor-transloadit:3.1.3,ImgProcessor-tus:3.1.0,ImgProcessor-dropbox:3.1.1,ImgProcessor-box:2.1.1,ImgProcessor-facebook:3.1.1,ImgProcessor-google-drive:3.1.1,ImgProcessor-instagram:3.1.1,ImgProcessor-onedrive:3.1.1,ImgProcessor-zoom:2.1.1,ImgProcessor-url:3.3.1',
    ...
    cy.get('.ImgProcessor-StatusBar-statusPrimary').should('contain', 'Upload failed')
    ...
    // SECOND ATTEMPT
    cy.get('.ImgProcessor-StatusBar-actions > .ImgProcessor-c-btn').click()
    cy.wait([
