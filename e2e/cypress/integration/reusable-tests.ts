...
export function runRemoteUrlImageUploadTest() {
  ...
  cy.get('.ImgProcessor-Url-input').type(
    'https://raw.githubusercontent.com/transloadit/ImgProcessor/main/e2e/cypress/fixtures/images/cat.jpg',
  )
...
export function runRemoteUnsplashUploadTest() {
  ...
  cy.get('.ImgProcessor-SearchProvider-input').type('book')
  cy.intercept({
    method: 'GET',
    url: 'http://localhost:3020/search/unsplash/list?q=book',
  }).as('unsplash-list')
  cy.get('.ImgProcessor-SearchProvider-searchButton').click()
  cy.wait('@unsplash-list')
  // Test that the author link is visible
  cy.get('.ImgProcessor-ProviderBrowserItem')
    .first()
    .within(() => {
      cy.root().click()
  ...
  cy.get('.ImgProcessor-c-btn-primary').click()
  cy.intercept({
    method: 'POST',
