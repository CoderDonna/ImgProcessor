...
async function compileCSS() {
  ...
  const files = await glob('packages/{,@ImgProcessor/}*/src/style.scss');
  ...
  for (const file of files) {
    ...
    const outfile = determineOutfile(outputDir, 'style.css', 'ImgProcessor.css');
...
function determineOutfile(outputDir, defaultName, packageName) {
  return outputDir.includes(path.normalize('packages/ImgProcessor/')) ?
